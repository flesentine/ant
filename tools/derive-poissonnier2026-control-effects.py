...
def main():
 ...
 rows=mod.records(mod.sheets(sys.argv[1])['Exp 1']);rows=[r for r in rows if r['Pheromone']=='n'];colonies=sorted({int(float(r['Colony'])) for r in rows});rng=random.Random(20260901)
 result={'schema_version':1,'source':'poissonnier2026_final_record','source_xlsx_sha256':'b311d5fdc89eac56724bb5195743cf4bb52a6cff4040b18704353091e1fe6318','scope':'open_arena_DCM_controls_only','contrast':'short_20cm_minus_long_100cm','status':'descriptive_model_screening_only_not_fitting','bootstrap':{'method':'independent within-treatment nonparametric bootstrap of per-ant summary rows','replicates':20000,'seed':20260901,'rng':'Python random.Random','warning':'Descriptive screening interval only; it does not model colony clustering.'},'metrics':{}}
