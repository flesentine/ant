...
function simulateTreatment(c,t,trials,seed0,base,policy,mechanism='P3'){
  if(mechanism!=='P3'&&mechanism!=='absolute')throw new Error('Unknown P3 estimator mechanism '+mechanism+'.');
...
function searchPanel(ref,cols,policy,base,{trials,seed0,panel='P3',retainAll=false}){
  if(panel!=='P3'&&panel!=='absolute')throw new Error('Unknown P3 estimator panel '+panel+'.');
...
function assertPolicySemantics(p){
  ...
  if(p.id!=='P3_response_estimation_v1'||p.status!=='development_response_estimation_policy_frozen_before_estimator_implementation_or_response_target_semantic_access')throw new Error('Unexpected P3 policy identity/status.');
  const e=p.response_parameter_surface.estimated_parameters;
  if(!eq(p.response_parameter_surface.estimated_parameter_names_exact,['sigma_field_mm','kappa_trail_per_s'])||!eq(p.response_parameter_surface.nuisance_parameters,[]))throw new Error('Frozen P3 parameter surface changed.');
  if(!eq(e.sigma_field_mm.bounds,[2,32])||e.sigma_field_mm.scale!=='log'||!eq(e.kappa_trail_per_s.bounds,[0,16])||e.kappa_trail_per_s.scale!=='linear'||e.kappa_trail_per_s.nested_canonical_null_value!==0)throw new Error('Frozen P3 parameter bounds/scales changed.');
  if(!eq(p.reference_partition.colonies,[0,7,16,20,21,27])||p.reference_partition.folds!==6||!String(p.reference_partition.training_rule).includes('equal-weight arithmetic mean of the five colony-level contrasts'))throw new Error('Frozen P3 LOCO partition/weighting changed.');
  if(!eq(p.primary_fit_observables.map(x=>x.name),PRIMARY_METRICS)||p.primary_objective.candidate_ranking_uses_only_primary_objective!==true||p.primary_objective.same_objective_for_P3_and_absolute_benchmark!==true)throw new Error('Frozen P3 primary objective changed.');
  if(!eq(p.secondary_guard_observables,SECONDARY_METRICS)||p.secondary_guard_policy.ranking_use!==false||p.secondary_guard_policy.selection_use!==false||p.secondary_guard_policy.sample_sd_denominator!=='n-1')throw new Error('Frozen P3 secondary guards changed.');
  const s=p.search_protocol,a=s.absolute_transduction_benchmark_panel;
  if(s.candidate_budget_per_fold_total!==1000||s.positive_P3_candidates!==999||s.exact_canonical_null_candidates!==1||a.candidate_budget_per_fold_total!==1000||a.positive_absolute_candidates!==999||a.exact_canonical_null_candidates!==1)throw new Error('Frozen P3 candidate panels changed.');
  if(!eq(s.halton_mapping.map(x=>[x.prime,x.parameter]),HALTON)||s.training_trials_per_treatment_path_per_candidate!==60||s.heldout_evaluation_trials_per_treatment_path!==120||s.physics_dt_s!==0.02||s.root_fit_seed!==6210000||s.root_evaluation_seed!==6810000||s.common_random_numbers!==true||!String(s.response_rng).startsWith('none in P3 and none in the absolute-transduction benchmark'))throw new Error('Frozen P3 search execution changed.');
  if(s.trial_seed_pairing.short_path_offset!==0||s.trial_seed_pairing.long_path_offset!==1000)throw new Error('Frozen P3 CRN path offsets changed.');
  const cv=p.cross_validation_and_survival;
  if(cv.canonical_null_survival_guard.minimum_heldout_fold_wins!==5||cv.canonical_null_survival_guard.folds_total!==6||cv.canonical_null_survival_guard.median_relative_improvement_must_be_strictly_positive!==true)throw new Error('Frozen P3 null survival guard changed.');
  if(cv.structural_increment_survival_guard.minimum_heldout_fold_wins_vs_selected_absolute_benchmark!==5||cv.structural_increment_survival_guard.folds_total!==6||cv.structural_increment_survival_guard.median_relative_improvement_vs_absolute_benchmark_must_be_strictly_positive!==true)throw new Error('Frozen P3 structural survival guard changed.');
  const f=p.final_all_data_fit;
  if(f.authorized_only_if_both_survival_guards_pass!==true||f.training_trials_per_treatment_path_per_candidate!==120||f.fit_seed!==7210000||f.final_check_trials_per_treatment_path!==240||f.final_check_seed!==7610000)throw new Error('Frozen P3 final-fit execution changed.');
  if(p.nested_models_and_comparators.absolute_transduction_structural_benchmark.not_a_P1_rerun!==true||p.nested_models_and_comparators.absolute_transduction_structural_benchmark.P1_official_result_semantics_loaded!==false)throw new Error('Frozen P3 absolute comparator firewall changed.');
  if(p.estimator_implementation_gate.high_resolution_response_search_authorized!==false||p.estimator_implementation_gate.response_target_semantic_access_authorized!==false||p.promotion_rule.canonical_locomotion_update_authorized!==false||p.promotion_rule.ymaze_unlock_authorized!==false)throw new Error('Frozen P3 authorization firewall changed.');
  if(p.global_firewalls.P1_official_result_semantic_access!==false||p.global_firewalls.P2_official_result_semantic_access!==false||p.global_firewalls.P2_lapse_structure!==false||p.global_firewalls.Y_maze_access!==false)throw new Error('Frozen P3 upstream/Y-maze firewall changed.');
  if(!near(p3.FIXED_DT,s.physics_dt_s,1e-15))throw new Error('Frozen P3 physics dt does not match runtime.');
  return true;
}
