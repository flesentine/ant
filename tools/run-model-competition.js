...
function runCompetition({trials=400,firstSeed=928491,dt=FIXED_DT}={}){
  const shortBundle=loadBundle('open_arena_short_control.json'),longBundle=loadBundle('open_arena_long_control.json');
  const hypothesis=readJson(path.resolve(__dirname,'..','hypotheses','open_arena_locomotion_context_v1.json'));
  const reference=readJson(path.resolve(__dirname,'..','reference','poissonnier2026_control_effects.json'));
...
module.exports={runCompetition,METRICS,PRIMARY};
