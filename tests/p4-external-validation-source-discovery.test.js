...
assert.strictEqual(blob(discoveryRel),'a5c150d8910020bf6c89f04ecefeb769cf2f1003','external-validation source-discovery blob drift');
...
assert.strictEqual(d.status,'published_source_screen_complete_no_uncontaminated_promotion_grade_holdout_new_prospective_dataset_required');
...
assert.strictEqual(d.discovery_conclusion.eligible_existing_published_source_count_after_blinding_filter,0);
...
assert.strictEqual(d.criteria_contract_resolution.resolution,'fail_closed_no_eligible_existing_source_instead_of_forcing_an_ineligible_selection');
