...
for(const id of ['oberhauser_wendt_czaczkes_2020_lasius_concentration_attraction','perna_et_al_2012_linepithema_relative_turning_with_detection_threshold_boundary','czaczkes_gruter_jones_ratnieks_2011_lasius_presence_strength_boundary','koch_czaczkes_2021_lasius_transient_nonfollowing','poissonnier_et_al_2026_lasius_context_boundary']) assert(byId[id],'missing independent source '+id);
...
assert.match(doseFacts,/73\.4%/);assert.match(doseFacts,/85\.1%/);assert.match(doseFacts,/94\.4%/);assert.match(doseFacts,/51\.2%/);
...
assert.strictEqual(perna.species,'Linepithema humile');assert.match(perna.frozen_relevant_facts.join(' '),/minimum sensory detection threshold/);assert.match(perna.allowed_inference,/absolute-signal-dependent expression or detection stage/);assert.match(perna.limits.join(' '),/cross-species/);
const strength=byId.czaczkes_gruter_jones_ratnieks_2011_lasius_presence_strength_boundary;
assert.match(strength.frozen_relevant_facts.join(' '),/one ant passage to 20 ant passages/);assert.match(strength.allowed_inference,/need not be assumed to form one universal smooth dose-response/);
const lapse=byId.koch_czaczkes_2021_lasius_transient_nonfollowing;
assert.match(lapse.allowed_inference,/already-tested P2-v1 signal-independent lapse class is not a fresh P4 class/);assert.match(lapse.limits.join(' '),/does not establish that lapse probability depends on instantaneous absolute pheromone concentration/);
const context=byId.poissonnier_et_al_2026_lasius_context_boundary;
assert.match(context.frozen_relevant_facts.join(' '),/No significant modulation/);assert.match(context.limits.join(' '),/cannot be used to create path-length-specific, direction-specific, or recent-experience-specific P4 response parameters/);
...
assert.match(c.description,/absolute local pheromone-evidence quantity/);assert.match(c.independent_evidence_strength,/direct_Lasius_niger_support_for_absolute_concentration_dependence/);
assert(c.why_genuinely_new_relative_to_closed_versions.some(x=>/Unlike P1-v1/.test(x)));assert(c.why_genuinely_new_relative_to_closed_versions.some(x=>/Unlike P2-v1/.test(x)));assert(c.why_genuinely_new_relative_to_closed_versions.some(x=>/Unlike P3-v1/.test(x)));
const subclasses=c.mechanism_subclasses_not_selected_by_this_gate.join(' ');
assert.match(subclasses,/hard absolute detection threshold/);assert.match(subclasses,/smooth deterministic absolute-signal gate/);assert.match(subclasses,/stochastic detection or engagement/);assert.match(subclasses,/regularized relative comparison/);
const unset=c.must_remain_unset_until_a_separate_mechanism_freeze.join(' ');
assert.match(unset,/exact absolute-evidence statistic/);assert.match(unset,/exact transduction equation/);assert.match(unset,/sensor geometry/);assert.match(unset,/parameter values and search bounds/);assert.match(unset,/estimation objective, folds, seeds, trial budgets, comparators, and promotion criteria/);
const closed=Object.fromEntries(e.previously_tested_or_nonfresh_classes.map(x=>[x.maps_to,x]));
for(const version of ['P1-v1','P2-v1','P3-v1']){assert(closed[version],'missing closed prior class '+version);assert.match(closed[version].status,/already_tested_and_permanently_closed_not_a_fresh_P4_class/);}
const neg=Object.fromEntries(e.negative_or_not_yet_supported_classes.map(x=>[x.class_id,x]));
for(const id of ['path_distance_direction_recent_experience_specific_response','stable_specialist_pheromone_ignorer_identity','pheromone_driven_speed_law_as_primary_P4_mechanism','repulsive_or_sign_reversing_pheromone_response']) assert(neg[id],'missing negative/not-supported class '+id);
...
for(const [k,v] of Object.entries(e.selection_firewall)){if(k==='rule')continue;assert.strictEqual(v,false,k+' must remain false');}
assert.match(e.next_gate,/separate P4 candidate-class decision record/);assert.match(e.next_gate,/exact absolute-evidence statistic/);assert.match(e.next_gate,/unset until a later prospective mechanism freeze/);
...
const d=read(decisionRel);assert.strictEqual(d.independent_evidence_input.git_blob_sha,'262f332062f271bbf111d0572f83b2faaf2cfd74');assert.strictEqual(d.selected_next_candidate_class.class_id,c.class_id);assert.strictEqual(d.selection_firewall.P4_executable_mechanism_exists,false);
...
const m=read(mechanismRel);assert.strictEqual(m.candidate_class_input.evidence_git_blob_sha,'262f332062f271bbf111d0572f83b2faaf2cfd74');assert.strictEqual(m.candidate_class_input.selected_class_id,c.class_id);assert.strictEqual(m.implementation_gate.src_p4_exists_at_this_freeze,false);assert.strictEqual(m.implementation_gate.implementation_authorized_by_this_record,false);
...
if(implementationPresent){
  ...
  const rp=read('hypotheses/p4_reachability_execution_v1.json'),a=read('hypotheses/p4_implementation_authorization_v1.json');
  assert.strictEqual(rp.mechanism_freeze.git_blob_sha,'609551836e540c341365db9cc987d2ca340cc053');assert.strictEqual(a.reachability_execution_policy.git_blob_sha,'0d452f9c0d548ec962e0a6d9826648bf5e14a4ef');assert.strictEqual(a.still_forbidden.response_target_access,true);assert.strictEqual(a.still_forbidden.reserved_Y_maze_access,true);
}
...
console.log('p4-candidate-class-evidence.test.js PASS '+JSON.stringify({evidence_blob:blob(rel),sources:e.independent_sources.length,supported_class:c.class_id,historical_formally_selected:false,later_decision_present:true,later_mechanism_present:true,later_implementation_present:implementationPresent,later_response_policy_present:responsePolicyPresent,reserved_ymaze_access:false}));
