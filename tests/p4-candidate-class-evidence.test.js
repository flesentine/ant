...
assert.match(dose.frozen_relevant_facts.join(' '),/73\.4%/);
assert.match(dose.frozen_relevant_facts.join(' '),/85\.1%/);
assert.match(dose.frozen_relevant_facts.join(' '),/94\.4%/);
...
assert.match(c.mechanism_subclasses_not_selected_by_this_gate.join(' '),/hard absolute detection threshold/);
assert.match(c.mechanism_subclasses_not_selected_by_this_gate.join(' '),/smooth deterministic absolute-signal gate/);
assert.match(c.mechanism_subclasses_not_selected_by_this_gate.join(' '),/stochastic detection or engagement/);
assert.match(c.must_remain_unset_until_a_separate_mechanism_freeze.join(' '),/exact transduction equation/);
assert.match(c.must_remain_unset_until_a_separate_mechanism_freeze.join(' '),/sensor geometry/);
...
assert.strictEqual(e.evidence_synthesis.candidate_class_formally_selected,false,'historical evidence gate itself did not select a class');
for(const [k,v] of Object.entries(e.selection_firewall)) if(k!=='rule') assert.strictEqual(v,false,k+' historical evidence firewall must remain false');
...
assert.strictEqual(fs.existsSync(path.join(root,'src/p4.js')),false,'P4 runtime still must not exist');
assert.strictEqual(fs.existsSync(path.join(root,'models/lasius_niger_painted_trail_p4_v1.json')),false,'P4 model still must not exist');
assert.strictEqual(fs.existsSync(path.join(root,'hypotheses/p4_response_estimation_v1.json')),false,'P4 estimation policy still must not exist');

console.log('p4-candidate-class-evidence.test.js PASS '+JSON.stringify({evidence_blob:blob(rel),supported_class:c.class_id,historical_formally_selected:false,later_decision_present:decisionPresent,later_mechanism_present:mechanismPresent,p4_runtime_present:false,reserved_ymaze_access:false}));
