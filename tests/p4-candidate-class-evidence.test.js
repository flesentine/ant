...
if(implementationPresent){
  ...
  const p=read('hypotheses/p4_reachability_execution_v1.json'),a=read('hypotheses/p4_implementation_authorization_v1.json');
  assert.strictEqual(p.mechanism_freeze.git_blob_sha,'609551836e540c341365db9cc987d2ca340cc053');assert.strictEqual(a.reachability_execution_policy.git_blob_sha,'0d452f9c0d548ec962e0a6d9826648bf5e14a4ef');assert.strictEqual(a.still_forbidden.response_target_access,true);assert.strictEqual(a.still_forbidden.reserved_Y_maze_access,true);
}
...
console.log('p4-candidate-class-evidence.test.js PASS '+JSON.stringify({evidence_blob:blob(rel),sources:e.independent_sources.length,supported_class:c.class_id,historical_formally_selected:false,later_decision_present:true,later_mechanism_present:true,later_implementation_present:implementationPresent,reserved_ymaze_access:false}));
