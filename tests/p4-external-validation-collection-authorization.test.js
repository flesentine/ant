...
assert.strictEqual(blob(authRel),'74411bd6d0ce1d71abff5a1e212853d4c3846e2f','collection authorization blob drift');
...
assert.strictEqual(blob(trialRel),'a37a83c407e00a3dd2c15417334c2e78d61ab476','trial schema blob drift');
...
for(const field of ['session_id','video_file_id','scorer_identity']) assert.ok(trial.required_fields.includes(field),'trial schema missing session linkage field '+field);
...
assert.ok(rules.includes('equal or otherwise cannot yield a unique earliest terminal'),'ambiguous earliest entry must be technical exclusion');
assert.ok(rules.includes('release_position_error_mm > 0.2 or release_heading_error_rad > 0.025')&&rules.includes('exclusion_flag must be true'),'release-pose tolerance failure must force exclusion');
