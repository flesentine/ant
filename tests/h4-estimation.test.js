...
assert.throws(()=>h4.validateReferenceInputs({root,policy,target:inputs.target,cal:badCal,sourceManifest:inputs.sourceManifest,h2Report:inputs.h2Report,h3Report:inputs.h3Report,targetPath:inputs.targetPath}),/Y-maze/);
...
console.log('h4-estimation.test.js PASS '+JSON.stringify({policy_blob:pinned.sha,qualification:q.status,colonies:inputs.colonies.length}));
