...
assert.match(m.mechanism.absolute_detection.gate,/S <= theta_detect/);assert.match(m.mechanism.absolute_detection.gate,/S > theta_detect/);
...
assert.strictEqual(m.mechanism.sector_sensors.radius_mm,10);assert.strictEqual(m.mechanism.sector_sensors.digital_operator.radial_bins,4);assert.strictEqual(m.mechanism.sector_sensors.digital_operator.angular_bins_per_sector,8);assert.strictEqual(m.mechanism.sector_sensors.digital_operator.samples_per_sector,32);
assert.strictEqual(m.mechanism.field.type,'gaussian_distance_to_segment');assert.match(m.mechanism.field.equation,/dose_ratio \* exp/);
assert.strictEqual(m.mechanism.stochastic_structure.signal_independent_lapse_state_present,false);assert.strictEqual(m.mechanism.stochastic_structure.response_rng_present,false);assert.strictEqual(m.mechanism.stochastic_structure.biology_rng_draws_added,0);
...
assert.strictEqual(W(0.10,0.15,0.25),0);assert(W(0.10,0.16,0.25)>0);assert.strictEqual(W(0.40,0.40,0.25),0);assert(Math.abs(W(0.10,0.40,0.25)+W(0.40,0.10,0.25))<1e-15);assert(Math.abs(W(0.20,0.60,0.25)-W(0.40,1.20,0.25))<1e-15);assert.strictEqual(W(0.02,0.08,0.25),0);assert(W(0.08,0.32,0.25)!==0);
const eng=m.engineering_only_reachability_values;
assert.strictEqual(eng.sigma_field_mm,8);assert.strictEqual(eng.kappa_trail_per_s,4);assert.strictEqual(eng.theta_detect,0.25);assert.strictEqual(eng.planned_reference_free_panel.physics_dt_s,0.02);assert.deepStrictEqual(eng.planned_reference_free_panel.exact_identity_seeds,[840001,840002,840003,840004,840005,840006,840007,840008]);assert.strictEqual(eng.planned_reference_free_panel.nominal_reachability_trials,400);assert.strictEqual(eng.planned_reference_free_panel.nominal_first_seed,841000);assert.strictEqual(eng.planned_reference_free_panel.nominal_last_seed,841399);assert.strictEqual(eng.retuning_from_reachability_forbidden,true);assert(2*0.10<eng.theta_detect);assert.match(eng.value_rationale,/not a biological threshold estimate/i);assert.match(eng.value_rationale,/may not define a later estimation bound/i);
...
assert.strictEqual(m.implementation_gate.src_p4_exists_at_this_freeze,false);assert.strictEqual(m.implementation_gate.p4_model_exists_at_this_freeze,false);assert.strictEqual(m.implementation_gate.new_P4_simulation_executed_at_this_freeze,false);assert.strictEqual(m.implementation_gate.implementation_authorized_by_this_record,false);
...
const forbiddenText=m.invariances_and_forbidden_inputs.join(' ');assert.match(forbiddenText,/No stochastic detection or response RNG/);assert.match(forbiddenText,/No sigmoid slope, Hill exponent, epsilon/);assert.match(forbiddenText,/No painted-trail-driven speed modulation/);assert.match(forbiddenText,/No reserved Y-maze/);
...
if(implementationPresent){
  ...
  const policy=read('hypotheses/p4_reachability_execution_v1.json'),auth=read('hypotheses/p4_implementation_authorization_v1.json');
  assert.strictEqual(policy.mechanism_freeze.git_blob_sha,'609551836e540c341365db9cc987d2ca340cc053');assert.strictEqual(auth.reachability_execution_policy.git_blob_sha,'0d452f9c0d548ec962e0a6d9826648bf5e14a4ef');assert.strictEqual(auth.authorization.create_src_p4_js,true);assert.strictEqual(auth.still_forbidden.response_target_access,true);assert.strictEqual(auth.still_forbidden.reserved_Y_maze_access,true);
}
assert.strictEqual(fs.existsSync(path.join(root,'hypotheses/p4_response_estimation_v1.json')),false);assert.strictEqual(fs.existsSync(path.join(root,'tools/run-p4-estimation.js')),false);
console.log('p4-mechanism-selection.test.js PASS '+JSON.stringify({mechanism_blob:blob(mechPath),subclass:m.selection_rule.selected_subclass,theta_engineering:eng.theta_detect,historical_implementation_authorized:false,later_implementation_present:implementationPresent,response_target_access:false}));
