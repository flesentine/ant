...
const branch=h5e.currentBranchName(root);let authorization;if(branch==='main'){authorization=h5e.assertHighResolutionAuthorized(root);}else{assert.throws(()=>h5e.assertHighResolutionAuthorized(root),/not effective until merged to main/);authorization=h5e.assertHighResolutionAuthorized(root,null,{branchName:'main'});}assert.strictEqual(authorization.high_resolution_search_authorized,true);assert.strictEqual(authorization.estimator_git_blob_sha,'7d4a68f024c09c111a38088be2c943b7de74b464');
