...
const preBlob='edaae2a2b2601fb0957287b3d76b1e4029683ea5';
const workflowBlob='d650438d88055fa5e91f45c72e71d5498ba8fed8';
const authBlob='8cf7ddb3218836aec1e18eaf7f3ea0c3c2a92a6b';
const expectedBefore='71ef3857e775d11269246990b907994e3442e3b2';
...
assert.strictEqual(p.authorization_main_commit,expectedBefore);
assert.strictEqual(p.authorization_git_blob_sha,authBlob);
assert.deepStrictEqual(p.permanent_main_workflow,{
  name:'Test and deploy ANTLAB',workflow_file:'.github/workflows/pages.yml',main_commit:expectedBefore,
  run_id:34795850404,run_number:168,event:'push',conclusion:'success',
  test_job_id:103828674565,test_job_conclusion:'success',deploy_job_id:103828787871,deploy_job_conclusion:'success'
});
...
assert.deepStrictEqual(p.execution_trigger,{
  event:'push',branch:'main',path:workflowRel,expected_before_commit:expectedBefore,
  pull_request_enabled:false,workflow_dispatch_enabled:false,workflow_run_rerun_enabled:false,later_unrelated_main_push_may_rerun:false
});
assert.match(p.execution_trigger_rule,/event\.before commit is exactly/i);
assert.match(p.execution_trigger_rule,/paths filter is only an additional trigger reduction/i);
...
assert.deepStrictEqual(p.ephemeral_fields_permitted_to_change,['official_stage_A_execution_authorized','official_stage_A_execution.official_stage_A_execution_authorized']);
...
assert.deepStrictEqual(p.official_execution_contract,{
  candidate_index:307,sigma_field_mm:18.319554310908863,kappa_trail_per_s:6.342935528120713,theta_detect:0.9184,
  left_marked_seed_root:8210000,right_marked_seed_root:8210000,marked_trials_per_side:1000,paired_left_right_common_random_numbers:true,
  neutral_seed_root:8310000,neutral_trials:1000,total_trials:3000,duration_s:90,workers:1,
  adaptive_trial_budget_authorized:false,seed_change_authorized:false,valid_result_rerun_authorized:false
});
assert.deepStrictEqual(p.stage_A_result_contract,{
  report_id:'P4_Y_maze_consistency_stage_A_simulation_v1',mode:'official',scientific_evidence:false,
  interpretation:'posthoc_same_study_cross_apparatus_descriptive_consistency_only',validation_pass_fail_threshold:null,promotion_rule:null,
  canonical_update_authorized:false,result_must_be_accepted_exactly_as_produced:true,
  report_and_provenance_must_be_frozen_in_separate_post_execution_gate_before_stage_B:true
});
assert.match(p.rerun_policy,/event\.before=71ef3857/i);
assert.match(p.rerun_policy,/GITHUB_RUN_ATTEMPT=1/);
assert.match(p.rerun_policy,/separate invalidation record/i);
for(const k of ['biological_Y_maze_summary_access_authorized_during_stage_A','raw_Y_maze_choice_access_authorized_during_stage_A','colony_level_Y_maze_outcome_access_authorized_during_stage_A','real_stage_B_execution_authorized','stage_B_authorized','P4_parameter_change_authorized','P4_runtime_change_authorized','P4_refit_or_retuning_authorized','canonical_promotion_authorized'])assert.strictEqual(p[k],false,k+' must remain false');
...
// Static trigger/firewall proof: review/manual/rerun and any push after the intended main transition fail closed.
...
assert.match(workflow,/EXPECTED_BEFORE='71ef3857e775d11269246990b907994e3442e3b2'/);
...
assert.match(workflow,/expected_before_commit!==EXPECTED_BEFORE/);
assert.match(workflow,/provenance\.workflow_attempt!==1/);
assert.match(workflow,/provenance\.workflow_event_before/);
