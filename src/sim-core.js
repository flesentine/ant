(function (root, factory) {
  ...
  const BIOLOGY_KEYS = new Set([
    ...
    'locomotor_activation', 'history_distance_scale_mm', 'activation_tau_s', 'max_speed_gain_fraction',
  ...
  class Ant {
    ...
    update(dt,sim){if(this.finished)return;this.contactFlash=Math.max(0,this.contactFlash-dt);const m=sim.model.movement;if(this.pauseRemaining<=0&&hazard(Number(m.pause_rate_s)*this.pauseScale,dt,this.rng)){this.pauseRemaining=Number(m.pause_min_s)+this.rng.next()*(Number(m.pause_max_s)-Number(m.pause_min_s));this.state='paused';}if(this.pauseRemaining>0){this.pauseRemaining-=dt;if(this.pauseRemaining<=0)this.state='moving';this.recordTail(dt);return;}const theta=Number(m.speed_reversion_rate_s),sigmaSpeed=Number(m.speed_noise_sigma_sqrt_s);this.speedFactor+=theta*(1-this.speedFactor)*dt+sigmaSpeed*Math.sqrt(dt)*this.rng.normal();this.speedFactor=clamp(this.speedFactor,.6,1.4);if(typeof sim.beforeHeadingNoise==='function')sim.beforeHeadingNoise(this,dt);this.heading=normalizeAngle(this.heading+this.rng.normal()*Number(m.angular_sigma_rad_sqrt_s)*this.turnScale*Math.sqrt(dt));const speedMultiplier=Number.isFinite(Number(this.speedMultiplier))&&Number(this.speedMultiplier)>0?Number(this.speedMultiplier):1,actualSpeed=this.baseSpeed*this.speedFactor*speedMultiplier,ox=this.x,oy=this.y;let nx=ox+Math.cos(this.heading)*actualSpeed*dt,ny=oy+Math.sin(this.heading)*actualSpeed*dt;if(!pointInGeometry(nx,ny,sim.apparatus.geometry)){const mode=sim.apparatus.boundary.mode;if(mode==='terminate'){const exit=lastInsidePoint(ox,oy,nx,ny,sim.apparatus.geometry),moved=Math.hypot(exit.x-ox,exit.y-oy);this.x=exit.x;this.y=exit.y;this.distanceTravelled+=moved;if(moved>0)this.movingTime+=dt;this.finish(sim.apparatus.boundary.outcome||'boundary_exit',sim.time+dt,sim,exit);this.recordTail(dt);return;}const normal=closestBoundaryNormal(nx,ny,sim.apparatus.geometry);this.heading=normalizeAngle(reflectHeading(this.heading,normal.nx,normal.ny));this.wallContacts++;sim.metrics.wallContacts++;sim.events.push({time:sim.time,type:'wall_contact',ant:this.id});nx=ox+Math.cos(this.heading)*actualSpeed*dt;ny=oy+Math.sin(this.heading)*actualSpeed*dt;if(!pointInGeometry(nx,ny,sim.apparatus.geometry)){nx=ox;ny=oy;}}const moved=Math.hypot(nx-ox,ny-oy);this.x=nx;this.y=ny;this.distanceTravelled+=moved;if(moved>0)this.movingTime+=dt;this.state='moving';const terminal=terminalAt(this.x,this.y,sim.apparatus.terminal_regions);if(terminal)this.finish(terminal.label||'terminal',sim.time+dt,sim,{x:this.x,y:this.y});this.recordTail(dt);}
  ...
  class Simulation {
    ...
    fingerprint(){return JSON.stringify({experiment:this.experiment.id,model_hash:this.provenance.model_hash,apparatus_hash:this.provenance.apparatus_hash,time:Number(this.time.toFixed(6)),metrics:this.metrics,ants:this.ants.map(a=>[a.id,Number(a.x.toFixed(6)),Number(a.y.toFixed(6)),Number(a.heading.toFixed(6)),Number(a.speedFactor.toFixed(6)),Number((a.speedMultiplier||1).toFixed(6)),a.state,a.outcome,a.contacts,a.wallContacts])});}
  }
